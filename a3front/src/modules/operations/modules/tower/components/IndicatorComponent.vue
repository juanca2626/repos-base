<template>
  <a-card :class="['service-card', cardStyle]" :bordered="true">
    <div class="service-content" style="padding: 0; margin: 10px 0">
      <a-row :gutter="[16, 8]" style="display: flex; align-items: center">
        <a-col style="display: flex; justify-content: center; align-items: center; height: 100%">
        ...
        <a-col>
          <a-typography-title
            :class="['service-title', titleStyle]"
            :level="5"
            @click="handleClick(action)"
          >
            {{ total }} servicios
          </a-typography-title>
          <a-typography-text class="service-subtitle" type="secondary">
            {{ subtitle }}
          </a-typography-text>
        </a-col>
        <a-col>
          <ArrowRightOutlined style="color: #bd0d12; font-weight: bold; font-size: 25px" />
        </a-col>
      </a-row>
    </div>
    <div class="service-buttons">
      <template v-if="module === 'service-management'">
        <a-button
          ...
          :key="index"
          type="default"
          @click="handleClick(action, btn.type)"
