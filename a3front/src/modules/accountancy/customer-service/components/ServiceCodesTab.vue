<template>
  <div class="service-codes-tab-content">
    ...
    <div class="service-codes-list-section">
      ...
      <a-spin :spinning="loading">
        ...
        <div v-else class="service-codes-table">
          <a-table :columns="columns" :data-source="serviceCodes" :pagination="false" size="small">
...
<script setup>
  ...
  const { loading, saving, addServiceCode, removeServiceCode } = useClaimsApi();
  ...
  const handleAddServiceCode = async () => {
    ...
    const nextLine =
      props.serviceCodes.length > 0 ? Math.max(...props.serviceCodes.map((s) => s.NROLIN)) + 1 : 1;
