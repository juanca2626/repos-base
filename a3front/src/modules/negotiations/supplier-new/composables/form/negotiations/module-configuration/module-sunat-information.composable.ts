...
export function useModuleSunatInformationComposable() {
    ...
    try {
      ...
      const { percentage = 0 } =
        data.supplier_progress_bar.find(
          (e: any) => e.module_id === SupplierProgressModulesEnum.SUNAT_INFORMATION
        ) || {};
      handleProgressFormSpecific(FormComponentEnum.MODULE_SUNAT_INFORMATION, true, percentage);
      handleIsEditFormSpecific(FormComponentEnum.MODULE_SUNAT_INFORMATION, true);
  ...
  const fetchSunatInformation = async () => {
    ...
    if (data?.id) {
      ...
      handleProgressFormSpecific(FormComponentEnum.MODULE_SUNAT_INFORMATION, true, 13);
    } else {
      handleIsEditFormSpecific(FormComponentEnum.MODULE_SUNAT_INFORMATION, false);
