...
export const useExchangeRatesList = (props: { filters: FilterDatesInterface }) => {
  const [modal, contextHolder] = Modal.useModal();

  const editSettingIgv = (record: ExchangeRatesResponseInterface) => {
  ...
  const data = ref<ExchangeRatesResponseInterface[]>([]);
  ...
  const fetchIgvData = async (page: number = 1, pageSize: number = 10) => {
    ...
    try {
      const response = await supportApi.get('exchange-rates', {
        params: {
          page,
          per_page: pageSize,
    ...
    modal.confirm({
      ...
      async onOk() {
        try {
          ...
          await supportApi.delete(`exchange-rates/${id}`);
          await fetchIgvData();
        } catch (error) {
