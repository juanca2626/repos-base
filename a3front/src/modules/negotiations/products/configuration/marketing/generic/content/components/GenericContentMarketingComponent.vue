<template>
  <div class="generic-content-marketing-component mb-5">
          ...
          <div class="marketing-read-sections">
            <div class="marketing-read-field">
              <div
                ...
                v-html="formState.itinerary.text"
              />
              <div v-else class="editor-container">
                <EditorQuillComponent
                  placeholder=""
                  class="custom-editor"
                  :model-value="formState.itinerary.text"
                  @update:model-value="(val: string) => (formState.itinerary.text = val)"
                />
              </div>
            </div>
          </div>
      ...
      >
        ...
        <div class="mt-4 menu-card-container">
          <a-card size="small" class="card-text-type card-menu">
            <template #title>
              <div class="card-title-with-help">
                <div v-if="isMenuHelpOpen" class="help-inline-content card-help-in-title">
                  <div class="help-overlay-title">Pautas para redactar menú</div>
                  <ul class="help-overlay-list">
                    <li>Aun no definido</li>
                  </ul>
                  <!-- <div class="help-overlay-subtitle">Ejemplos:</div>
                  <div class="help-overlay-example">
                    <div class="texto-itinerario">Desayuno: Jugo de frutas naturales, café o té, pan con mantequilla y mermelada, huevos revueltos. Almuerzo: Ensalada mixta, lomo saltado con arroz, agua mineral. Cena: Sopa de verduras, pescado a la plancha con vegetales, postre de la casa.</div>
                  </div> -->
                </div>
                <div class="card-title-row">
                  <div class="card-title-row-center">
                    ...
                    <span
                      class="icon-help-trigger"
                      @click="toggleMenuHelpOverlay"
                      role="button"
                      tabindex="0"
                      @keydown.enter="toggleMenuHelpOverlay"
                    >
                      <IconHelp />
                    </span>
                  </div>
                  <a-button
                    type="text"
                    class="card-text-type-action"
                    @click="markAsReviewedMenu"
                    v-if="formState.menu.status === 'PENDING'"
                  >
                    <span class="mr-2">Marcar como revisado</span>
                    <IconCircleCheck />
                  </a-button>
                  ...
                  <ReviewStatusBadge
                    v-if="formState.menu.status === 'REVIEWED'"
                    status="reviewed"
                  />
                </div>
              </div>
        ...
        <div class="mt-4 summary-card-container">
          <a-card size="small" class="card-text-type card-summary">
            <template #title>
              <div class="card-title-with-help">
                <div v-if="isSummaryHelpOpen" class="help-inline-content card-help-in-title">
                  <div class="help-overlay-title">Pautas para redactar Summary</div>
                  <ul class="help-overlay-list">
                    <li>
                      Según lo ingresado por Loading categorizar en el orden descrito en el detalle
                      del
                    </li>
                    <!-- <li>Incluir información esencial: qué incluye, duración, aspectos destacados.</li>
                    <li>Mantener un tono informativo y directo, evitando redundancias.</li> -->
                  </ul>
                  <!-- <div class="help-overlay-subtitle">Ejemplos:</div>
                  <div class="help-overlay-example">
                    <div class="texto-itinerario">Servicio privado de 5 días que incluye traslados, alojamiento en Treehouse Lodge, visitas guiadas a Machu Picchu y Ollantaytambo. Incluye desayunos y un almuerzo. Experiencia centrada en la cultura inca y los paisajes andinos.</div>
                  </div> -->
                </div>
                <div class="card-title-row">
                  <div class="card-title-row-center">
                    ...
                    <span
                      class="icon-help-trigger"
                      @click="toggleSummaryHelpOverlay"
                      role="button"
                      tabindex="0"
                      @keydown.enter="toggleSummaryHelpOverlay"
                    >
                      <IconHelp />
                    </span>
                  </div>
                  <a-button
                    type="text"
                    class="card-text-type-action"
                    @click="markAsReviewedSummary"
                    v-if="formState.summary.status === 'PENDING'"
                  >
                    <span class="mr-2">Marcar como revisado</span>
                    <IconCircleCheck />
                  </a-button>
                  ...
                  <ReviewStatusBadge
                    v-if="formState.summary.status === 'REVIEWED'"
                    status="reviewed"
                  />
                </div>
              </div>
...
<script setup lang="ts">
  ...
  import { useGenericContentMarketingComposable } from '../composables/useGenericContentMarketingComposable';
  ...
  } = useGenericContentMarketingComposable();
</script>

<style scoped lang="scss">
  @import '@/scss/_variables.scss';

  .generic-content-marketing-component {
    overflow-x: hidden;
    max-width: 100%;
  }
  ...
  .generic-content-marketing-component .editor-container {
    :deep(.editor-quill-container) {
      padding-left: 0 !important;
      padding-right: 0 !important;
    }
