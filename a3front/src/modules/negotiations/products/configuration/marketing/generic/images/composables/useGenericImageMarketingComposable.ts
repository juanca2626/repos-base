...
export const useGenericImageMarketingComposable = () => {
  const navigationStore = useNavigationStore();
  const { activeTabKey, getSectionsCodeActive, getSectionsActiveTypeItem } =
  ...
  const isEditingContent = computed(() => {
    return getSectionsActiveTypeItem.value?.editing ?? false;
  });
  ...
  const handleSave = () => {
    ...
    if (tabKey != null && sectionCode != null) {
      setCompletedItem(tabKey, sectionCode, null);
    }
  };
