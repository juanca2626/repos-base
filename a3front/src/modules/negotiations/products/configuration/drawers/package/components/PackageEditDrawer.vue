<template>
  ...
  >
    <a-form layout="vertical">
      <a-alert
        v-if="availableProgramsToOperate.length === 0"
        type="warning"
        show-icon
      ...
      <a-form-item>
        ...
        <a-select
          ...
          mode="multiple"
          :options="availableProgramsToOperate"
          :max-tag-count="3"
          v-model:value="formState.programsToOperateIds"
          :disabled="availableProgramsToOperate.length === 0"
        >
...
<script setup lang="ts">
  ...
  import { usePackageEditDrawer } from '../composables/usePackageEditDrawer';
  ...
  } = usePackageEditDrawer(emit, props);
</script>
