...
export const useSchedule = (currentKey?: string, currentCode?: string) => {
  ...
  const initializeFromStore = () => {
    if (currentKey && currentCode) {
      const storedData = packageConfigurationStore.getServiceDetailsSchedule(
        currentKey,
        currentCode
      );
      if (storedData) {
  ...
  const resetSchedules = () => {
    ...
    if (currentKey && currentCode) {
      packageConfigurationStore.setServiceDetailsSchedule(currentKey, currentCode, {
  ...
  const handleSaveTwentyFourHours = () => {
    // Los días ya tienen el estado aplicado desde handleToggleDayCheckbox
    // Solo necesitamos salir del modo de selección
    // Los checkboxes permanecen visibles y seleccionados para los días con 24 horas
    isSelectingTwentyFourHours.value = false;
  };
