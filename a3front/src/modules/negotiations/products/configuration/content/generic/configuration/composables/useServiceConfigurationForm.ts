...
export const useServiceConfigurationForm = ({
  formState,
  ...
  const buildCapacityConfigurationRequest = (): CapacityConfigurationRequest => {
    const serviceDetailId = genericStore.getServiceDetailId(props.currentKey, props.currentCode);
  ...
  const updateStoreAfterSave = (savedConfig: any) => {
    ...
    genericStore.updateCapacityConfiguration(savedConfig);

    if (activeTabKey.value && getSectionsCodeActive.value && getSectionsItemActive.value?.id) {
      setCompletedItem(
        activeTabKey.value,
        getSectionsCodeActive.value,
        getSectionsItemActive.value.id
      );
    }
  };
  ...
  const handleSave = async () => {
    try {
      ...
      const existingConfig = genericStore.getCapacityConfiguration(
        props.currentKey,
        props.currentCode
      );
