...
import { useAirlineFilterStore } from '../store/airline-filter.store';

export function useAirlineFilterForm(emit: DrawerEmitTypeInterface) {
  const { filterState, resetFilterState, setApplyFilters } = useAirlineFilterStore();
  ...
  const fetchResources = async () => {
    try {
      ...
      const response = await supplierApi.get('suppliers/airlines/filter-resources', {
        params: {
