...
} from '@/modules/negotiations/suppliers/transports/interfaces';
...
export function useTransportList() {
  ...
  const transportFilterStore = useTransportFilterStore();
  const { filterState, resetApplyFilters } = transportFilterStore;
  const { applyFilters } = storeToRefs(transportFilterStore);
  const drawer = useDrawerStore();
  const { inactive } = useInactiveResource('transports');

  const isLoading = ref<boolean>(false);
  const infoInactive = ref<SupplierTransport | null>(null);
  ...
  const data = ref<SupplierTransport[]>([]);
  ...
  const fetchListData = async (page: number = 1, pageSize: number = 10) => {
    ...
    try {
      ...
      const { data } = await supplierApi.post<ApiListResponse<SupplierTransportResponse[]>>(
        `suppliers/${selectedClassificationId.value}`,
        {
          ...
          vehicle_types: filterState.vehicleTypes,
        }
      );
      ...
      console.error('Error fetching transport list data:', error);
    } finally {
      isLoading.value = false;
    }
  };

  const transformListData = (responseData: SupplierTransportResponse[]) => {
  ...
  const handleInactive = (row: SupplierTransport) => {
