...
export function useTransportFilterForm(emit: DrawerEmitTypeInterface) {
  const { filterState, resetFilterState, setApplyFilters } = useTransportFilterStore();
  ...
  const fetchResources = async () => {
    try {
      ...
      const response = await supplierApi.get('suppliers/transports/filter-resources', {
        params: {
