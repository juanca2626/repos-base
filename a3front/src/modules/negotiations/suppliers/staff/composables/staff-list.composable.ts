...
export function useStaffList() {
  ...
  const staffFilterStore = useStaffFilterStore();
  const { filterState, resetApplyFilters } = staffFilterStore;
  const { applyFilters } = storeToRefs(staffFilterStore);
  const drawer = useDrawerStore();
  const { inactive } = useInactiveResource('staff');

  const isLoading = ref<boolean>(false);
  const infoInactive = ref<SupplierStaff | null>(null);
  ...
  const fetchListData = async (page: number = 1, pageSize: number = 10) => {
    ...
    try {
      ...
      const { data } = await supplierApi.post<ApiListResponse<SupplierStaffResponse[]>>(
        `suppliers/${selectedClassificationId.value}`,
        {
          ...
          staff_roles: filterState.staffRoles,
        }
      );
  ...
  const transformListData = (responseData: SupplierStaffResponse[]) => {
  ...
  const handleInactive = (row: SupplierStaff) => {
