...
export function useStaffFilterForm(emit: DrawerEmitTypeInterface) {
  const { filterState, resetFilterState, setApplyFilters } = useStaffFilterStore();
  ...
  const fetchResources = async () => {
    try {
      ...
      const response = await supplierApi.get('suppliers/staff/filter-resources', {
        params: {
