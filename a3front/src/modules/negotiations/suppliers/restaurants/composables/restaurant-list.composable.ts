...
import type {
  SupplierRestaurant,
  SupplierRestaurantResponse,
} from '@/modules/negotiations/suppliers/restaurants/interfaces';
...
export function useRestaurantList() {
  ...
  const restaurantFilterStore = useRestaurantFilterStore();
  const { filterState, resetApplyFilters } = restaurantFilterStore;
  const { applyFilters } = storeToRefs(restaurantFilterStore);
  const drawer = useDrawerStore();
  const { inactive } = useInactiveResource('restaurants');

  const isLoading = ref<boolean>(false);
  const infoInactive = ref<SupplierRestaurant | null>(null);
  ...
  const data = ref<SupplierRestaurant[]>([]);
  ...
  const fetchListData = async (page: number = 1, pageSize: number = 10) => {
    ...
    try {
      ...
      if (filterState.searchTerm) body.search_term = filterState.searchTerm;
      if (filterState.status?.length) body.status = filterState.status;
      if (filterState.countryStateKeys?.length)
        body.country_state_keys = filterState.countryStateKeys;
      if (filterState.chains?.length) body.chains = filterState.chains;
      if (filterState.amenities?.length) body.amenities = filterState.amenities;
      if (filterState.typeFoods?.length) body.type_foods = filterState.typeFoods;
      ...
      const { data } = await supplierApi.post<ApiListResponse<SupplierRestaurantResponse[]>>(
  ...
  const transformListData = (responseData: SupplierRestaurantResponse[]) => {
  ...
  const handleInactive = (row: SupplierRestaurant) => {
