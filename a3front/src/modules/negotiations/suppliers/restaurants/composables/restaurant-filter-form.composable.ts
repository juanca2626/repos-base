...
export function useRestaurantFilterForm(emit: DrawerEmitTypeInterface) {
  const { filterState, resetFilterState, setApplyFilters } = useRestaurantFilterStore();

  const isLoading = ref<boolean>(false);

  const collapseSectionKeys = ref<string[]>([
    'chains',
    'country-states',
    'status',
    'type-foods',
  ...
  const typeFoods = ref<SelectOption[]>([]);
  const amenities = ref<SelectOption[]>([]);
  ...
  const fetchResources = async () => {
    try {
      ...
      const response = await supplierApi.get('suppliers/restaurants/filter-resources', {
        params: {
          keys: ['chains', 'country_state_options', 'type_foods', 'amenities', 'status'],
        },
      });
      ...
      typeFoods.value = mapItemsToOptions(response.data.data.type_foods);
      amenities.value = mapItemsToOptions(response.data.data.amenities);
