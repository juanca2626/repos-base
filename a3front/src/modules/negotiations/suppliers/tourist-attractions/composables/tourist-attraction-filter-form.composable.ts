...
export function useTouristAttractionFilterForm(emit: DrawerEmitTypeInterface) {
  const { filterState, resetFilterState, setApplyFilters } = useTouristAttractionFilterStore();
  ...
  const fetchResources = async () => {
    try {
      ...
      const response = await supplierApi.get('suppliers/tourist-attractions/filter-resources', {
        params: {
