...
export function useWaterTransportList() {
  const waterTransporFilterStore = useWaterTransportFilterStore();
  const { filterState, resetApplyFilters } = waterTransporFilterStore;
  const { applyFilters } = storeToRefs(waterTransporFilterStore);
