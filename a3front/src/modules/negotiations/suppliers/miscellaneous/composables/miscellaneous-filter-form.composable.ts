...
export function useMiscellaneousFilterForm(emit: DrawerEmitTypeInterface) {
  const { filterState, resetFilterState, setApplyFilters } = useMiscellaneousFilterStore();
  ...
  const fetchResources = async () => {
    try {
      ...
      const response = await supplierApi.get('suppliers/miscellaneous/filter-resources', {
        params: {
