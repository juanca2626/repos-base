...
import type {
  SupplierMiscellaneous,
  SupplierMiscellaneousResponse,
} from '@/modules/negotiations/suppliers/miscellaneous/interfaces';
...
export function useMiscellaneousList() {
  const router = useRouter();
  const miscellaneousFilterStore = useMiscellaneousFilterStore();
  const { filterState, resetApplyFilters } = miscellaneousFilterStore;
  const { applyFilters } = storeToRefs(miscellaneousFilterStore);
  const drawer = useDrawerStore();
  const { inactive } = useInactiveResource('miscellaneous');

  const isLoading = ref<boolean>(false);
  const infoInactive = ref<SupplierMiscellaneous | null>(null);
  ...
  const data = ref<SupplierMiscellaneous[]>([]);
  ...
  const fetchListData = async (page: number = 1, pageSize: number = 10) => {
    ...
    try {
      ...
      const { data } = await supplierApi.post<ApiListResponse<SupplierMiscellaneousResponse[]>>(
        `suppliers/${selectedClassificationId.value}`,
        {
          ...
          item_types: filterState.itemTypes,
        }
      );
      ...
      console.error('Error fetching miscellaneous list data:', error);
    } finally {
      isLoading.value = false;
    }
  };

  const transformListData = (responseData: SupplierMiscellaneousResponse[]) => {
  ...
  const handleInactive = (row: SupplierMiscellaneous) => {
