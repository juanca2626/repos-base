<template>
    ...
    <template v-else>
      <div v-if="files.length > 0" class="files mb-3">
        <div class="" v-for="(file, index) in files" :key="index">
          <div style="background-color: #fafafa; padding: 20px; width: 100%; position: relative">
            ...
            <a-row>
              ...
              >
                <p style="color: #1890ff; font-size: 12px">{{ file.name }}</p>
              </a-col>
      ...
      <template v-else>
        ...
        <label :for="`file-${index}`">
          <span class="iborrainputfile"
            ><i class="bi bi-upload"></i> {{ t('global.label.select_file') }}
          </span>
        </label>
  ...
  <template v-else>
    ...
    <template v-else>
      <div class="file-item" v-if="link != '' && link != null">
        <span>{{ link }}</span>
      </div>
    </template>
  </template>
...
<script setup>
  ...
  function handleClickDeleteFile(index, single) {
    if (single) {
      if (props.link != '' && props.link != null) {
        uploadsStore.removeFile(link.value);
  ...
  const handleFileUploadDocument = ($event) => {
    ...
    console.log('Uploads: ', upload_files);
  ...
  const sendFilesToS3 = async () => {
    ...
    previewUrl.value = response[0].link;
    loading.value = false;
  };
</script>
