        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content" v-if="messages">
                ...
                <div class="modal-body">
                    <div class="modal-title">{{ translations.label.messages }}<span>({{messages.length}})</span></div>
                    <div class="modal-subtitle"><span>{{translations.label.details_communication}}.</span>
                    ...
                    <div class="modal-mensaje-card" :class="{'modal-mensaje-card-nuevo':verify_recent(m.updated_at)}"
                        ...
                        <div class="mensaje-card-contenido modal-mensaje-card-respondiendo">
                            ...
                            <div class="mensaje-card-textarea" style="padding-top: 11px;">
                                ...
                                <textarea rows="5" :placeholder="translations.label.comments" @keyup="update_message_edit(m)" v-model="m.message_edit" v-if="m.show_edit">{{ m.message_edit }}</textarea>
                            </div>
                                ...
                                <div class="mensaje-card-contenido">
                                    ...
                                    <div class="mensaje-card-adjunto-editando">
                                        ...
                                        <button class="button-cancelar" type="button" :disabled="loading"
                                                @click="m.show_reply=false; delete messages_replies[m.id]">
                            ...
                            <div class="mensaje-card-adjunto-editando" v-show="m.show_edit">
                                ...
                                <button class="button-cancelar" type="button" :disabled="loading" @click="m.show_edit=false; delete messages_edits[m.id]">
                                    {{ translations.label.cancel }}
                                </button>
...
<!--                            REPLY REPLY REPLY REPLY REPLY REPLY REPLY REPLY REPLY REPLY-->
                            <div v-for="mm in messages" v-if="mm.reply_id===m.id">
                                ...
                                <div class="mensaje-card-respuesta" :class="{'modal-mensaje-card-nuevo':verify_recent(mm.updated_at)}">
                                    ...
                                    <div class="mensaje-card-contenido">
                                        ...
                                        <div class="mensaje-card-textarea" style="padding-top: 11px;">
                                            ...
                                            <textarea rows="5" :placeholder="translations.label.comments" @keyup="update_message_edit(mm)" v-model="mm.message_edit" v-if="mm.show_edit">{{ mm.message_edit }}</textarea>
                                        </div>
                                    </div>
                                ...
                                <div class="mensaje-card-adjunto-editando" v-show="mm.show_edit">
                                    ...
                                    <button class="button-cancelar" type="button" :disabled="loading" @click="mm.show_edit=false; delete messages_edits[mm.id]">{{translations.label.cancel}}</button>
...
<script>
    export default {
        ...
        computed: {
            messages(){
                let messages_ =
                    (this.$store.state.rooms.room.data_messages !== undefined )
                        ? this.$store.state.rooms.room.data_messages
                        : []
                messages_.forEach(m=>{
                    if( this.messages_replies[m.id] !== undefined ){
                        m.show_reply = true
                    } else {
                        m.show_reply = false
                    }
                    if( this.messages_edits[m.id] !== undefined ){
                        m.show_edit = true
                        m.message_edit = this.messages_edits[m.id]
                    } else {
                        m.show_edit = false
                    }
                })
        ...
        methods: {
            ...
            update_message_edit(message){
                this.messages_edits[message.id] = message.message_edit
                console.log(this.messages_edits[message.id])
            },
            will_remove(message){
                ...
                axios.delete(
                    ...
                    .then((result) => {
                        if( result.data.success ){
                            ...
                            //     position: 'top-right'
                            // })
                            if( this.messages_replies[message_id] !== undefined ){
                                delete this.messages_replies[message_id]
                            }
                            this.search(true)
                            let message_ = this.translations.messages.has_removed_a_message
            ...
            set_show_new_message(){
                ...
                this.messages.forEach( m=>{
                    ...
                    m.show_reply = false
                    if( this.messages_replies[m.id] !== undefined ){
                        delete this.messages_replies[m.id]
                    }
                    if( this.messages_edits[m.id] !== undefined ){
                        delete this.messages_edits[m.id]
                    }
                } )
            },
            set_show_reply(message){
                ...
                if( message.show_reply ){
                    ...
                    delete this.messages_replies[message.id]
                } else {
                    this.messages.forEach( m=>{
                        m.show_edit = false
                        m.show_reply = false
                        if( m.id === message.id ){
                            m.message_edit = m.message
                            this.messages_replies[message.id] = m.message
                        } else {
                            if( this.messages_replies[m.id] !== undefined ){
                                delete this.messages_replies[m.id]
                            }
            ...
            set_show_edit(message){
                if( message.show_edit ){
                    ...
                    delete this.messages_edits[message.id]
                } else {
                    this.messages.forEach( m=>{
                        m.show_edit = false
                        m.show_reply = false
                        if( m.id === message.id ){
                            m.message_edit = m.message
                            this.messages_edits[message.id] = m.message
                        } else {
                            if( this.messages_replies[m.id] !== undefined ){
                                delete this.messages_replies[m.id]
                            }
            ...
            search(socket=false){
                ...
                axios.get(
                    baseExternalURL + 'api/message?entity='+this.entity+'&object_id='
                                +this.master_sheet_id + '&limit=100'
                )
                    .then((result) => {
                        this.loading = false
                        // this.messages = result.data.data
                        this.$store.commit("rooms/set_data", ['data_messages', result.data.data], {root:true})
                        if(socket){
                            this.$socket.emit('rooms/refresh', this.master_sheet_id, 'data_messages', result.data.data)
                        }
                    })
                    .catch((e) => {
                        console.log(e)
            ...
            save_message(){
                ...
                let data = {
                    entity: this.entity,
                    object_id: this.master_sheet_id,
                ...
                axios.post(
                    ...
                    .then((result) => {
                        if( result.data.success ){
                            // this.$toast.success(this.translations.label.sent, {
                            //     position: 'top-right'
                            // })
                            let message_ = ''
                            ...
                            this.$parent.send_notification(message_)
                            this.search(true)
                            this.$parent.master_sheet.messages_count+=1
                        } else{
                            this.$toast.error('Error', {
                                position: 'top-right'
                            })
