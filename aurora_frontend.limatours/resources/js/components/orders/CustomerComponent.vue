            ...
            <div class="d-flex justify-content-between align-items-center">
                ...
                <div class="col">
                    <div class="form-group">
                        ...
                        <b-form-select v-model="team" :options="teams" v-bind:disabled="loading" class="form-control ml-1">
                        </b-form-select>
                    </div>
                </div>
                <div class="col">
                    <div class="form-group">
                        <label><strong>Kam</strong></label>
                        <v-select :reduce="kams => kams.code" :options="kams"
                                v-model="kam" @search="searchKams"
            ...
            <div class="p-4" v-if="quantity > 0 && !loading">
                <div v-if="!detail">
                    <div class="table-responsive">
                        <table class="datatable table text-center table-facturacion-customer">
                        <thead>
                        <tr>
                        ...
                        <tbody>
                        <tr v-for="(customer, k) in customers">
                            <td><a href="javascript:;" v-on:click="showDetail('customer', k, customer)">{{ customer.orders[0].RAZON }} ({{ k }})</a></td>
                    ...
                    <download-excel :data="customers_export" :disabled="loading"
                        :fields="json_fields" worksheet="Reporte"
                        v-if="customers_export.length > 0"
                        name="customer_report.xls" class="btn btn-primary btn-md">
                        Descargar datos en Excel
                    </download-excel>
                    <!-- button type="button" v-on:click="downloadExcel('customer_report')" v-if="quantity > 0 && !loading" class="btn btn-primary btn-md">Descargar datos en Excel</button -->
                </div>
                <div v-if="detail">
                    <div class="table-responsive" v-if="quantityExecutive > 0">
                        <table class="table table-hover text-center table-facturacion-customer-resume">
                            <thead>
                            <tr>
                    ...
                    <div class="table-responsive" v-if="quantityCustomer > 0">
                        <table class="table text-center table-facturacion-customer-resume">
                            <thead>
                            <tr>
                                <th scope="col">CLIENTE</th>
                                <th scope="col">PEDIDOS RECIBIDOS</th>
                    ...
                    <div class="table-responsive" v-if="quantityProduct > 0">
                        <table class="table text-center table-facturacion-customer-resume" v-if="quantityProduct > 0">
                            <thead>
                            <tr>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            searchKams: function (search, loading) {
                axios.post(
                    baseURL + 'board/kams', {
                        lang: this.lang,
                        kam: (search != undefined) ? search : ''
                    }
                )
                .then((result) => {
                    console.log(result.data.kams)
            ...
            renderReport: async function (orders) {
                ...
                vm.customers = await window.groupBy(orders, 'codigo')
                vm.all = await window.allStatsOrders(orders)
                vm.quantity = orders.length

                if(vm.quantity > 0)
                {
                    vm.customers_export = await window.getData(vm.customers)
                    vm.showTable(vm)
                }
            },
            showTable: function (vm) {
                ...
                setTimeout(function() {
                    $('.table-facturacion-customer').DataTable({
                        language: {
            ...
            downloadExcel: function (_type, _table) {
                window.location = baseURL + 'export_excel?type=' + _type + '&table=' + ((_table != undefined) ? _table : '');
            }
        }
    };
</script>
