<template>
    <section class="container" v-show="show_best_seller">
        <h1 class="sub-title">
            <b-skeleton-wrapper :loading="loading_best_seller">
                <template #loading>
                    <b-skeleton width="100%" height="10%"></b-skeleton>
                </template>
                <span v-if="sales_from === 'client'"> {{translations.label.bestsellers}} </span>
                <span v-if="sales_from === 'market'"> {{translations.label.bestsellers}} </span>
            </b-skeleton-wrapper>
        </h1>
        <b-skeleton-wrapper :loading="loading_best_seller">
            <template #loading>
            ...
            <div class="content-cards">
                <div class="grid">
                    <div class="grid__item" v-for="best_seller in package_best_sellers">
                        <div class="card">
                            <img class="card__img"
                                ...
                                <div class="card__price d-flex justify-content-between">
                                    ...
                                    <div class="text-right price">
                                        ...
                                        <div class="price__value">{{ translations.label.from }} <span class="icon-dollar-sign"></span> {{
                                           getPrice(best_seller.amounts.price_per_adult.room_dbl) }}
                                        </div>
    ...
    export default {
        data: () => {
            return {
                ...
                loading_best_seller: false,
                show_best_seller: true,
                package_best_sellers: [],
                lang: 'en',
                client_id: '',
                sales_from: '',
                rooms: {
        ...
        methods: {
            ...
            getBestSellers () {
                ...
                axios.post(
                    baseExternalURL + 'services/client/packages/best_sellers',
                    data,
                ).then((result) => {
                    this.loading_best_seller = false
                    this.package_best_sellers = result.data.data
                    this.sales_from = result.data.sales_from
                    if (this.package_best_sellers.length === 0) {
                        this.show_best_seller = false
                    }
                }).catch((e) => {
                    this.loading_best_seller = false
                    console.log(e)
