                    ...
                    <div class="table-responsive-sm">
                        <table class="table text-center canSelectText">
                            ...
                            <tr>
                                ...
                                <th scope="col" class="text-muted cursor-pointer" @click="filterBy('date_start')">
                                    Fecha de inicio  <i :class="{'fa':true,'fa-angle-down':filter_date_start,'fa-angle-up':!filter_date_start}"></i>
                                </th>
                                <th scope="col" class="text-muted cursor-pointer" @click="filterBy('paxs')">
                                    Paxs  <i :class="{'fa':true,'fa-angle-down':filter_paxs,'fa-angle-up':!filter_paxs}"></i>
                                </th>
                                ...
                                <th scope="col" class="text-muted cursor-pointer" @click="filterBy('passenger_email')">
                                    Email  <i :class="{'fa':true,'fa-angle-down':filter_passenger_email,'fa-angle-up':!filter_passenger_email}"></i>
                                </th>
                                <th scope="col" class="text-muted cursor-pointer" @click="filterBy('booking_state')">
                                    Estado  <i :class="{'fa':true,'fa-angle-down':filter_booking_state,'fa-angle-up':!filter_booking_state}"></i>
                                </th>
                                ...
                                <th scope="col" class="text-muted cursor-pointer" @click="filterBy('booking_type')">
                                    Tipo  <i :class="{'fa':true,'fa-angle-down':filter_booking_type,'fa-angle-up':!filter_booking_type}"></i>
                                </th>
                            ...
                            <tr v-for="(booking, b) in bookings">
                                <td>
                                    <button @click="closeOthersPopovers(booking)" class="btn btn-info" :id="'pop' + booking.id"
                                ...
                                <td class="td-body">
                                    ...
                                    <template v-if="flag_status == booking.id">
                                        <v-select class="form-control px-3 p-0" style="height:auto; width:150px;"
                                            v-model="booking.booking_state" :options="booking_status"
                                            label="name" code="code" v-on:input="saveBookingStatus(b)">
                                        </v-select>
                                    </template>
                                </td>
                                <td class="td-body">
                                    <span class="btn-estado" v-if="booking.service!==null">
                                        @{{ booking.aurora_code }}
                                    </span>
                                    <template v-else>
                                        <template v-if="booking.codes != undefined && booking.codes.length > 1">
                                            <div class="alert alert-warning">
                                                <small>Asegúrese que el código de este servicio esté creado en el backend</small>
                                            </div>
                                            <v-select class="form-control px-3 p-0" style="height:auto; width:150px;" v-model="booking.aurora_code"
                                                :options="booking.codes" label="code" v-on:input="saveBookingService(b)">
                                            </v-select>
                                        </template>
                                        <template v-else>
                                            <span class="btn-estado-danger btn-estado" style="background-color: #ea8181 !important;"
                                                title="Asegúrese que el código de este servicio esté creado en el backend">
                                                @{{ booking.aurora_code }}
                                            </span>
                                        </template>
                                    </template>
...
@section('js')
    <script>
        ...
        new Vue({
            ...
            mounted() {
                if (localStorage.getItem('servicesPentagrama')) {
                    try {
                        componentsChoosed = JSON.parse(localStorage.getItem('servicesPentagrama'))
                        this.componentsChoosed = componentsChoosed
                        this.componentsChoosedCount = componentsChoosed.length
                    } catch(e) {
                        localStorage.removeItem('servicesPentagrama')
                    }
                }
                this.onUpdate()
            },
            ...
            methods: {
                saveBookingStatus: function (_booking) {
                    console.log(this.bookings[_booking])

                    this.loading = true
                    axios.post('/api/channel/pentagrama/'+this.bookings[_booking].id+'/status_external', {
                        code: this.bookings[_booking].booking_state
                    })
                        .then((result) => {
                ...
                saveBookingService: function (_booking) {
                    console.log(this.bookings[_booking])

                    this.loading = true
                    axios.post('/api/channel/pentagrama/'+this.bookings[_booking].id+'/service', {
                        code: this.bookings[_booking].aurora_code,
                    })
                        .then((result) => {
                            if(result.data.success){
                                this.onUpdate()
                ...
                viewClosed(){
                    console.log(this.checkViewClosed)
                ...
                toggleStatus(id, status) {
                    this.loading = true
                    axios.post('/api/channel/pentagrama/'+id+'/status', { status : status })
                        .then((result) => {
                            if(result.data.success){
                                this.onUpdate()
                            }
                ...
                saveLocalStorage() {
                    ...
                    this.componentsChoosedCount = componentsChoosed.length
                    localStorage.setItem('servicesPentagrama', parsed);
                    this.onUpdate()
                },
                validateBookings: function (_bookings) {
                    let vm = this

                    _bookings.forEach((item, i) => {
                        if(item.aurora_code.indexOf('o') > -1)
                        {
                            vm.$set(item, 'codes', [])

                            let codes = item.aurora_code.split('o')

                            codes.forEach((code, c) => {
                                item.codes.push(code.trim())
                            })
                        }
                    })
                ...
                onUpdate () {
                    this.loading = true
                    this.urlbookings = 'api/channel/pentagrama/list?filterBy='+this.checkViewClosed +
                    ...
                    axios.get(baseExternalURL + this.urlbookings)
                        .then((result) => {
                            ...
                            this.validateBookings(this.bookings)
                            this.loading = false
                ...
                newFile : function(typeCreation){
                    ...
                    if( err==0 ){
                        ...
                        if( err == 0 ){
                            ...
                            axios({
                                method: 'post',
                                url: '/api/channel/pentagrama/reserve',
                                data: data
                            })
                                .then((result) => {
                                    console.log(result)
                                    ...
                                    if( result.data.success ){
                                        ...
                                        this.onUpdate()
                                        localStorage.setItem('servicesPentagrama', '')
                                        this.hideModal()
                                    } else {
