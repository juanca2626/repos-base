@php
    // Inicializar un array para almacenar los totales de cada pasajero
    $totals = array_fill(0, count($data["passengers"]), 0);
            ...
            @if($service["type"]=="service")
                ...
                <tr>
                   ...
                   @foreach($service["passengers"] as $key => $passenger)
                        @php
                            $totals[$key] += floatval($passenger["amount"]);

                            $style = $imprimirFila ? 'height: 20px; border-top: 3px solid red;' : '';
                            if (!empty($passenger['price_dynamic'])) {
                                $style .= ' color: red;';
                            }
                        @endphp
                        <td {!! $style ? "style='$style'" : '' !!}>{{ $passenger["amount"] }}</td>
                    @endforeach
                </tr>
                            ...
                            @isset($amount["passengers"])
                                ...
                                @foreach( $amount["passengers"] as $key => $passenger)
                                    @php
                                        $totals[$key] += floatval($passenger["amount"]);
                                    @endphp
                                    <td style="{{ $estiloCelda }}">{{ $passenger["amount"] }}</td>
                                @endforeach
                            @endisset
                        </tr>
