...
trait CalculateCancellationlPolicies
{
    ...
    public function getCancellationPolicyByTypeFit($selected_policies_cancelation, $guest_quantity, $rooms_quantity)
    {
        ...
        if (!$selected_policy_cancelation) {
            $selected_policy_cancelation = $selected_policies_cancelation->first(function ($item) use ($rooms_quantity
            ) {
                // type_fit == 2 | ROOM
     ...
     * @return array
     */
    public function calculateCancellationlPolicies(
        Carbon $current_date,
        Carbon $check_in,
        Carbon $check_out,
        ...
        $selected_policy_cancelation = $this->getCancellationPolicyByTypeFit($_selected_policies_cancelation,
        ...
        foreach ($argsPenalty as $policies_rate) {
            ...
            if ($policies_rate["min_day"] == 0 and $policies_rate["max_day"] > 0) {
                ...
                $apply_date = $policies_rate["max_day"] == 0 ? $check_in : subDateDays($check_in,
                ...
                // primer dia de la penalidad
                $apply_date = $policies_rate["min_day"] == 0 ? $current_date : subDateDays($check_in,
                ...
                // ultimo dia de la penalidad
                $expire_date = $policies_rate["max_day"] == 0 ? $check_in : subDateDays($check_in,
                    $policies_rate["max_day"]);
            }
            ...
            if ($penalty_code == 'NOT_REFOUNDABLE') {
                ...
                $message = $this->getMassagePenalty($penalty_price, $days_remain, $apply_date->format('d-m-Y'),
        ...
        if ($apply_panties->count() == 0) {
            $apply_panties->add(collect([
                ...
                'message' => $this->getMassageNotRefoundable($total_amount_rate, 0, $current_date->format('d-m-Y'),
        ...
        if ($result['next_penalty']['message'] != 'NOT_REFOUNDABLE') {
            if ($result['next_penalty']['days_remain'] == 0) {
                $result['next_penalty']['message'] = $this->getMassageInPenalty($result['next_penalty']['penalty_price'],
                    $result['next_penalty']['days_remain'], $result['next_penalty']['apply_date'],
                    $result['next_penalty']['expire_date']);
            } else {
                $result['next_penalty']['message'] = $this->getMassageBeforePenalty($result['next_penalty']['penalty_price'],
                    $result['next_penalty']['days_remain'], $result['next_penalty']['apply_date'],
                    $result['next_penalty']['expire_date']);
            }
        }
