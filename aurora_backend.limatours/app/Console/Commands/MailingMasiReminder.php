...
/**
 * Class MailingMasiReminder
 * @package App\Console\Commands
 */
class MailingMasiReminder extends Command
{
    public $account_sid = '';
    public $auth_token = '';
     ...
     * @var string
     */
    protected $signature = 'mailing:masi-reminder';
    ...
    {
        $configuration = MasiConfiguration::where('status', '=', 1)->first(); $time = '18:00';
        $date = ''; $flag_schedule = 0; $time_survey = '09:00';
        $flag_time = FALSE; $flag_time_survey = FALSE;

        if(!empty(@$configuration->time))
        {
            $flag_schedule = $configuration->flag_schedule; $date = $configuration->date;
        ...
        if($flag_time OR $flag_time_survey)
        {
            $types = ['', 'weekly', 'day_before', 'daily', '', 'survey'];
            foreach ($types as $key => $value) {
                if ($value != '') {

                    if(($flag_time_survey AND $value == 'survey') OR
                        ($flag_time AND $value != 'survey'))
                    {
                        $this->storeLogs(0, 0, 'email', $value, null, 'Mailing', false);
                        $field = 'test_mailing_'.$value;

                        $pax = [
                            'id' => 1,
                            'nombre' => 'Luis Shepherd',
                            'email' => 'kluizsv@gmail.com',
                        ];

                        $this->$field($key, $value, [$pax], false);
                    }
                }
            }
            ...
            foreach ($types as $key => $value) {
                if ($value != '') {
                    ...
                    if(($flag_time_survey AND $value == 'survey') OR
                        ($flag_time AND $value != 'survey'))
                    {
                        $this->storeLogs(0, 0, 'whatsapp', $value, null, 'Mailing', false);
    ...
    public function sendMail($body_html, $email_to, $subject, $tag = [])
    {
        $config = Configuration::getDefaultConfiguration()
            ->setApiKey('api-key', config('services.sendinblue.masi_api_key_v3'));
        ...
        if (!empty($email_to)) {
            ...
            $sendSMTPEmail['sender'] = array('name' => 'MASI', 'email' => 'dti@masi.pe');
            ...
            $sendSMTPEmail['bcc'] = array(
                ...
                array('email' => 'lsv@limatours.com.pe', 'name' => 'lsv@limatours.com.pe'),
            );
    ...
    public function getLogo($client_code)
    {
    ...
    public function putTextSkeleton($destinos, $lang)
    {
        $language = Language::where('id', '=', $lang)
            ->where('state', '=', 1)->first(); $new_destinos = [];

        foreach($destinos as $key => $valor)
        {
            $valor = (array) $valor;

            $data = MasterService::with(['translations' => function ($query) use ($language) {
    ...
    public function getTextSkeleton($servicios, $lang)
    {
        ...
        foreach ($servicios as $clave => $valor) {
            try {
                ...
                $skeletons[$i]['horin'] = $valor['horin'];
                $skeletons[$i]['descrip'] = trim($response['translations'][0]['value']);
                $i++;

            } catch (\Exception $exception) {
                Log::debug('getTextoSkeleton: '.$valor['codsvs'], $services);
            }
        }
        $skeletons = $this->orderMultiDimensionalArray($skeletons, 'horin');
        return $skeletons;
    ...
    */
    ...
    public function test_mailing_weekly($_key, $type, $email = [], $isDev = false)
    {
        try
        {
            ...
            $date_now = date('d-m-Y');
            $date_future = strtotime('+8 days', strtotime($date_now));
            ...
            if (count($files) > 0) {
                $this->storeLogs(null, 0, 'email', $type, $this->encodeArrayWithJsonColumns($files),
                    'Busqueda de Files '.$date);
                $this->storeFileDetail($files);
                $_date = explode("/", $date);
                ...
                foreach ($files as $clave => $valor) {
                    try
                    {
                        ...
                        $this->storeLogs($valor['nroref'], 0, 'email', $type, json_encode($emails), 'Lista de Pasajeros');
                        if ($checkEmails) {
                            ...
                            $linkIcons = $this->getLinkIcons($lang);
                            ...
                            $params = [
                                'nroref' => $valor['nroref'],
                                'lang' => ($key_lang > 0) ? $key_lang : 2,
                            ];

                            $response = (array)$this->stellaService->masi_services_file($params);
                            $destinos = (array)json_decode(json_encode($response['data']), true);
                            $destinos = $this->putTextSkeleton($destinos, ($key_lang > 0) ? $key_lang : 2);

                            if (count($destinos) > 0) {
                                $this->storeLogs($valor['nroref'], 0, 'email', $type,
                                ...
                                foreach ($emails as $k => $val) {
                                    ...
                                    if (!empty($val['email'])) {
                                        try {
                                            ...
                                            $this->storeLogs($valor['nroref'], $val['id'], 'email', $type, json_encode($send),
                                            ...
                                            $this->storeLogs($valor['nroref'], $val['id'], 'email', $type, json_encode($data),
                                                'Error en el envio de Email: '.$val['email'], false);
                                        }
                                ...
                                $this->storeLogs($valor['nroref'], 0, 'email', $type, null,
                                    'No se encontraron servicios', false);
                            }
                        } else {
                            $this->storeLogs(trim($valor['nroref']), 0, 'email', $type, null,
                    ...
                    catch(\Exception $ex)
                    {
                        ...
                        $this->storeLogs($valor['nroref'], 0, 'email', $type, json_encode($response), 'Error File', false);
                    }
                }
            } else {
                $this->storeLogs(null, 0, 'email', $type, null, 'No existen files para el dia '.$date, false);
            }
        }
        catch(\Exception $ex)
        {
            ...
            $this->storeLogs(null, 0, 'email', $type, json_encode($response), 'Error', false);
        }
    }

    public function test_mailing_day_before($_key, $type, $email = [], $isDev = false)
    {
        try
        {
            $clients = $this->getClients($type);
            ...
            $date_now = date('d-m-Y');
            $date_future = strtotime('+2 days', strtotime($date_now));
            $date_future = date('d-m-Y', $date_future);
            $date = str_replace('-', '/', $date_future);
            ...
            if (count($files) > 0) {
                ...
                $this->storeLogs(null, 0, 'email', $type, null,
    ...
    public function test_mailing_daily($_key, $type, $email = [], $isDev = false)
    {
        try
        {
            $clients = $this->getClients($type);
            ...
            $date_now = date('d-m-Y');
            $date_future = strtotime('+1 days', strtotime($date_now));
            $date_future = date('d-m-Y', $date_future);
            $date = str_replace('-', '/', $date_future);
                            ...
                            if (count($destinos) > 0) {
                                ...
                                foreach ($emails as $k => $val) {
                                    ...
                                    if (count($servicios) == 0) {
                                        $this->storeLogs($valor['nroref'], 0, 'email', $type, json_encode($val),
                                            'El pasajero no tiene servicios asignados', false);
                                    ...
                                    if (!empty($val['email'])) {
                                        try {
                                            ...
                                            $this->storeLogs($valor['nroref'], 0, 'email', $type, json_encode($data),
                                                'Error en el envio de Email: '.$val['email'], false);
                                        }
    ...
    public function test_mailing_survey($_key, $type, $email = [], $isDev = false)
    {
        try
        {
            $clients = $this->getClients($type);
    ...
    public function build_message_wsp($tipo, $data, $name_pax, $lang, $days_diff = 0)
    {
        ...
        if ($tipo == 2) {
            ...
            if (isset($data['file']['pronostico']) AND count($data['file']['pronostico']) > 0) {
            ...
            $message .= ''."\r\n";
            $message .= __('masi.tit_protocolos') . ' *';
            $message .= ''."\r\n";
            $message .= __('masi.detalle_protocolos');
            $message .= ''."\r\n";
        ...
        if($tipo == 3)
        {
            if ($days_diff >= 0 AND $days_diff <= 3) {
                $message .= ''."\r\n";
    ...
    public function wsp_mailing_weekly($key, $type, $phone_number = [], $isDev = false)
    {
        try
        {
            ...
            if (count($files) > 0) {
                $this->storeLogs(null, 0, 'whatsapp', $type, $this->encodeArrayWithJsonColumns($files),
                    'Busqueda de Files '.$date);
                $this->storeFileDetail($files);
                $_date = explode("/", $date);
                $date = $_date[2].'-'.$_date[1].'-'.$_date[0];
                ...
                foreach ($files as $clave => $valor) {
                    try
                    {
                        ...
                        $this->storeLogs($valor['nroref'], 0, 'whatsapp', $type, json_encode($phone_numbers),
                            'Lista de Pasajeros');
                        if ($checkPhoneNumbers) {
                            ...
                            if (count($destinos) > 0) {
                                $this->storeLogs($valor['nroref'], 0, 'whatsapp', $type,
                                    $this->encodeArrayWithJsonColumns($destinos, 'services_pax'), 'Servicios del file');
                                //Guardo Log
                                $params = [
                                    'nroref' => $valor['nroref'],
                                ...
                                foreach ($phone_numbers as $k => $val) {
                                    ...
                                    if (!empty($number)) {
                                        ...
                                        $this->storeLogs($valor['nroref'], $val['id'], 'whatsapp', $type, json_encode($send),
                                            'Envio de WhatsApp: '.$val['celular'], (($send['sent'] == 'OK') ? 1 : 0));
                                        $inserts[] = array(
                                            'email' => $number,
                                            'nombre' => $val['nombre'],
                                            'status' => $send['sent'],
                                ...
                                if ($notFoundServices == false and count($inserts) > 0) {
                                    $params = [
                                        'type' => $key,
                                        'fecha' => date("d/m/Y"),
                                        'hora' => date("H:i"),
                                ...
                                $this->storeLogs($valor['nroref'], 0, 'whatsapp', $type, null,
                                    'No se encontraron servicios', false);
                            }
                        } else {
                            $this->storeLogs(trim($valor['nroref']), 0, 'whatsapp', $type, null,
                                'No se encontraron numeros telefonicos para los pasajeros', false);
                        }
                    }
                    catch(\Exception $ex)
                    {
                        ...
                        $this->storeLogs($valor['nroref'], 0, 'whatsapp', $type, json_encode($response), 'Error File', false);
                    }
                }
            } else {
                $this->storeLogs(null, 0, 'whatsapp', $type, null, 'No existen files para el dia '.$date, false);
            }
        }
        catch(\Exception $ex)
        {
            ...
            $this->storeLogs(null, 0, 'whatsapp', $type, json_encode($response), 'Error', false);
        }

    }

    public function wsp_mailing_day_before($key, $type, $phone_number = [], $isDev = false)
    {
        try
        {
            $clients = $this->getClientsWsp($type);
    ...
    public function wsp_mailing_daily($key, $type, $phone_number = [], $isDev = false)
    {
        try
        {
            $clients = $this->getClientsWsp($type);
                        ...
                        if ($checkPhoneNumbers) {
                            ...
                            if (count($destinos) > 0) {
                                ...
                                foreach ($phone_numbers as $k => $val) {
                                    ...
                                    $servicios = $this->searchServicesByDate($ciudades, $date);
                                    if (count($servicios) == 0) {
                                        $this->storeLogs($valor['nroref'], 0, 'whatsapp', $type, json_encode($val),
                                            'El pasajero no tiene servicios asignados', false);
                                        $notFoundServices = true;
                                        continue;
                                    }
    ...
    public function wsp_mailing_survey($key, $type, $phone_number = [], $isDev = false)
    {
        try
        {
            $clients = $this->getClientsWsp($type);
                        ...
                        if ($checkPhoneNumbers) {
                            ...
                            if (count($destinos) > 0) {
                                ...
                                foreach ($phone_numbers as $k => $val) {
                                    ...
                                    $nameExl = explode(',', $val['nombre']);
                                    $paxName = (count($nameExl) > 1) ? ucwords(strtolower($nameExl[1])) : ucwords(strtolower($nameExl[0]));
                                    $templateMessageWhatsApp = $this->build_message_wsp($key, $file, $paxName, $lang);
                                    if ($isDev) {
                                        $number = $val['email'];
                                    } else {
                                        $number = $val['celular'];
    ...
    public function getClients($type)
    {
        $clients = Client::select(
            ...
            ->where(function ($query) use ($type) {
                $query->orWhere('client_mailing.status', 0)
                    ->orWhere('client_mailing.'.$type, 0);
            })
            ->get();
    ...
    public function getClientsWsp($type)
    {
        $clients = Client::select(
            ...
            ->where(function ($query) use ($type) {
                $query->orWhere('client_mailing.status', 0)
                    ->orWhere('client_mailing.whatsapp', 0)
                    ->orWhere('client_mailing.'.$type, 0);
            })
            ->get();
    ...
    public function getLinkIcons($lang)
    {
        if ($lang == 'es') {
    ...
    public function getLinkAir($lang)
    {
        if ($lang == 'es') {
            $linkAir = "https://res.cloudinary.com/litomarketing/image/upload/c_scale,w_300/v1560430025/aurora/mailing/mapa_es.png";
        } else {
            if ($lang == 'it') {
                $linkAir = "https://res.cloudinary.com/litomarketing/image/upload/c_scale,w_300/v1560430025/aurora/mailing/mapa_en.png";
            } else {
                if ($lang == "pt") {
                    $linkAir = "https://res.cloudinary.com/litomarketing/image/upload/c_scale,w_300/v1560430025/aurora/mailing/mapa_en.png";
                } else {
    ...
    public function storeLogs($file, $nropax, $type_send, $type_message, $data, $message, $status_validation = true)
    {
        $newLog = new MasiActivityJobLogs();
        $newLog->file = $file;
        $newLog->nropax = $nropax;
    ...
    public function storeFileDetail($files = [])
    {
