...
class RangesExport implements FromView, WithEvents, WithTitle
{
    ...
    public function view(): View
    {
        ...
        // = "";
        $category = QuoteCategory::where('id', $this->quote_category_id)->where('quote_id',
            $this->quote_id)->with('type_class.translations')->first();
        ...
        foreach ($ranges_quote as $range_quote) {
            ...
            $amount_range = QuoteDynamicSaleRate::where('quote_category_id',
                ...
                $range_quote["from"])->where('pax_to', $range_quote["to"]);
        ...
        foreach ($quote_services as $quote_service) {
            $ranges = QuoteDynamicSaleRate::where('quote_category_id', $category["id"])->where('quote_service_id',
                $quote_service["id"])->orderBy('date_service')->orderBy('pax_from')->get();
        ...
        foreach ($quote_services_optional as $quote_service) {
            $ranges = QuoteDynamicSaleRate::where('quote_category_id', $category["id"])->where('quote_service_id',
                $quote_service["id"])->orderBy('pax_from')->get();
