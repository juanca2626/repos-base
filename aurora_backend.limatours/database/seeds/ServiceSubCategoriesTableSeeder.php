...
class ServiceSubCategoriesTableSeeder extends Seeder
{
    ...
    public function run()
    {
        $file_categories = File::get("database/data/service_sub_categories.json");
        $file_category_translations = File::get("database/data/service_sub_category_translations.json");
        ...
        DB::transaction(function () use ($categories, $category_translations, $created_at) {
            foreach ($categories as $category) {
                $newExperiences = new ServiceSubCategory();
                $newExperiences->id = $category['id'];
                $newExperiences->service_category_id = $category['service_category_id'];
