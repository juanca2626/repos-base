...
@foreach( $data["categories"] as $category)
    ...
    <table>
        ...
        @foreach( $services as $service)
            @if($service["type"]=="service")
                ...
                <tr>
                    ...
                    @isset($data["accommodations"])
                    @if(isset($data["accommodations"]["single"]) and $data["accommodations"]["single"]>0)
                        <td {!! $imprimirFila ? " style='height: 20px; border-top: 3px solid red;'  " : ""  !!} >{{ $service["single_import"] }}</td>
                    @endif
                    @if(isset($data["accommodations"]["double"]) and $data["accommodations"]["double"]>0)
                        <td {!! $imprimirFila ? " style='height: 20px; border-top: 3px solid red;'  " : ""  !!} >{{ $service["double_import"] }}</td>
                    @endif 
                    @if(isset($data["accommodations"]["triple"]) and $data["accommodations"]["triple"]>0)
                        <td {!! $imprimirFila ? " style='height: 20px; border-top: 3px solid red;'  " : ""  !!} >{{ $service["triple_import"] }}</td>
                    @endif                               
                    @endisset
                    ...
                    @foreach($service["amount"] as $amount)
                        ...
                        <tr>
                            ...
                            <td {!! $imprimirFila ? " style='height: 20px; border-top: 3px solid red;font-weight: bold;'  " : " style='font-weight: bold;' "  !!}  >
                                ...
                                {{-- {{ $service["service_rooms"][0]["rate_plan_room"]["room"]["room_type"]["type_room"]["translations"][0]["value"] }} --}}
                                {{ clearNameRoom($service["service_rooms"][0]["rate_plan_room"]["room"]["translations"][0]["value"]) }}
                                - {{ $service["service_rooms"][0]["rate_plan_room"]["rate_plan"]["meal"]["translations"][0]["value"] }}
                            </td>
                          
                            @isset($data["accommodations"])
                            @if(isset($data["accommodations"]["single"]) and $data["accommodations"]["single"]>0)
                                <td {!! $imprimirFila ? " style='height: 20px; border-top: 3px solid red;font-weight: bold;'  " : " style='font-weight: bold;' "  !!}>{{ $amount["single"] }}</td>
                            @endif
                            @if(isset($data["accommodations"]["double"]) and $data["accommodations"]["double"]>0)
                                <td {!! $imprimirFila ? " style='height: 20px; border-top: 3px solid red;font-weight: bold;'  " : " style='font-weight: bold;' "  !!}>{{ $amount["double"] }}</td>
                            @endif 
                            @if(isset($data["accommodations"]["triple"]) and $data["accommodations"]["triple"]>0)
                                <td {!! $imprimirFila ? " style='height: 20px; border-top: 3px solid red;font-weight: bold;'  " : " style='font-weight: bold;' "  !!}>{{ $amount["triple"] }}</td>
                            @endif                               
                            @endisset
        ...
        <tr>
            ...
            @isset($data["accommodations"])
            @if(isset($data["accommodations"]["single"]) and $data["accommodations"]["single"]>0)                
                <td style="font-weight: bold; text-align: center;">{{ $category["services_total"]["single"] }}</td>
            @endif
            @if(isset($data["accommodations"]["double"]) and $data["accommodations"]["double"]>0)                
                <td style="font-weight: bold; text-align: center;">{{ $category["services_total"]["double"] }}</td>
            @endif 
            @if(isset($data["accommodations"]["triple"]) and $data["accommodations"]["triple"]>0)                
                <td style="font-weight: bold; text-align: center;">{{ $category["services_total"]["triple"] }}</td>
            @endif                               
            @endisset
