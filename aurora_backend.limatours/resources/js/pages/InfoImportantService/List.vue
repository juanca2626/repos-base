        ...
        <table-server :columns="table.columns" :options="tableOptions" :url="urlInclusions" class="text-center"
            ...
            <div class="table-actions" slot="actions" slot-scope="props" style="padding: 5px;">
                <b-dropdown class="mt-2 ml-2 mb-0" dropleft size="sm">
                    ...
                    <router-link :to="'/featured_service/edit/'+props.row.id" class="nav-link m-0 p-0">
                        <b-dropdown-item-button class="m-0 p-0" v-if="$can('update', 'informationimportantservices')">
  ...
  export default {
    ...
    data: () => {
      return {
        inclusionName: '',
        inclusions: [],
        urlInclusions: '/api/featured?token=' + window.localStorage.getItem('access_token') + '&lang=' +
    ...
    methods: {
      ...
      remove () {
        API({
          method: 'DELETE',
          url: 'featured/' + this.inclusion_id
        })
          .then((result) => {
      ...
      onUpdate () {
        this.urlInclusions = '/api/featured?token=' + window.localStorage.getItem('access_token') + '&lang=' +
          localStorage.getItem('lang')
