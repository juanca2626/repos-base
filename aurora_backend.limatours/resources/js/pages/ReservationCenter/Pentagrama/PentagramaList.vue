            ...
            <table-server :columns="table.columns" style="margin-top: 10px" :options="tableOptions" @loaded="onLoaded"
                ...
                <div class="table-actions text-center" slot="info" slot-scope="props" style="padding: 5px">
                    ...
                    <b-popover class="canSelectText" :show.sync="props.row.popShow" :target="'pop' + props.row.id" title="Datos de Reserva" triggers="hover focus">
                        <strong>Código de Pentagrama.com:</strong>
...
<script>
  ...
  export default {
    ...
    created () {
      ...
      this.urlbookings = '/api/channel/pentagrama/list?filterBy='+this.checkViewClosed + '&queryCustom='+this.query_custom
    ...
    methods: {
      ...
      toggleStatus(id, status) {
        this.loading = true
        API.post('/channel/pentagrama/'+id+'/status', { status : status })
          .then((result) => {
      ...
      newFile : function(typeCreation){
        ...
        if( err==0 ){
            ...
            if( err == 0 ){
                ...
                API({
                  method: 'post',
                  url: '/channel/pentagrama/reserve/',
                  data: data
                })
                  .then((result) => {
                    console.log(result)
