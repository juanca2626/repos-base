<template>
    <div class="container-fluid">
        <table-server :columns="table.columns" :options="tableOptions" :url="urlRestrictions" class="text-center"
                      ref="table">
            ...
            <div class="table-actions" slot="actions" slot-scope="props" style="padding: 5px;">
                <b-dropdown class="mt-2 ml-2 mb-0" dropleft size="sm">
                    ...
                    <router-link :to="'/restrictions/edit/'+props.row.id" class="nav-link m-0 p-0">
                        <b-dropdown-item-button class="m-0 p-0" v-if="$can('update', 'restrictions')">
        ...
        <b-modal :title="restrictionName" centered ref="my-modal" size="sm">
    ...
    export default {
        ...
        data: () => {
            return {
                restrictionName: '',
                restrictions: [],
                urlRestrictions: '/api/restrictions?token=' + window.localStorage.getItem('access_token') + '&lang=' +
        ...
        methods: {
            showModal (restriction_id, restriction_name) {
                this.restriction_id = restriction_id
                this.restrictionName = restriction_name
            ...
            remove () {
                API({
                    method: 'DELETE',
                    url: 'restrictions/' + this.restriction_id
                })
                    .then((result) => {
                        if (result.data.success === true) {
                            ...
                            if (result.data.used === true) {
                                this.$notify({
                                    ...
                                    text: this.$t('restrictions.error.messages.used')
                                })
                            } else {
                                this.$notify({
                                    group: 'main',
                                    type: 'error',
                                    title: this.$t('global.modules.restrictions'),
                                    text: this.$t('restrictions.error.messages.requirement_delete')
                                })
                            }
            ...
            onUpdate () {
                this.urlRestrictions = '/api/restrictions?token=' + window.localStorage.getItem('access_token') + '&lang=' +
                    localStorage.getItem('lang')
