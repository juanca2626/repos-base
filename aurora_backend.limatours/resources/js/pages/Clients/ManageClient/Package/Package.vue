<template>
    <div class="row col-12">
            ...
            <ul class="style_list_ul" id="list_services" ref="list_services">
                <draggable :list="services">
                    <li :class="{'style_list_li':true, 'item':true, 'selected':service.selected}" :id="'service_'+index"
                        ...
                        <span class="style_span_li"><span class="alert alert-warning" style="padding:0px !important">[{{ service.aurora_code}}]</span> {{ service.name}} ({{ service.markup ? service.markup : porcentage}}%)</span>
                    </li>
                </draggable>
            </ul>
        </div>
        <div class="col-1 mt-4 mr-2">
            ...
            <div class="col-12">
                <button @click="moveAllServices()" class="btn btn-secondary mover_controls btn-block pr-3">
            ...
            <div class="col-12">
                <button @click="inverseAllServices()" class="btn btn-secondary mover_controls btn-block pr-3">
            ...
            <ul class="style_list_ul" id="list_services_selected" ref="list_services_selected">
                <draggable :list="services_selected" class="list-group">
                    <li :class="{'style_list_li':true, 'item':true, 'selected':service.selected}"
                        ...
                        <span class="style_span_li"><span class="alert alert-warning" style="padding:0px !important">[{{ service.aurora_code}}]</span> {{ service.name}}</span>
                    </li>
                </draggable>
            </ul>
        </div>
            ...
            <ul class="style_list_ul" id="list_rates"
                ...
                <draggable :list="rates">
                    <li :class="{'style_list_li':true, 'item':true, 'selected':rate.selected}" :id="'rate_'+index"
                        ...
                        <span
                            class="style_span_li">{{ rate.name}} ({{ rate.markup ? rate.markup : selectMarkup()}}%)</span>
                    </li>
                </draggable>
            </ul>
        </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getPeriods: function () {
        API.get('client_services/selectPeriod?lang=' + localStorage.getItem('lang') + '&client_id=' + this.$route.params.client_id)
          .then((result) => {
      ...
      getServicesSelected: function () {
        API({
          ...
          .then((result) => {
            ...
            this.$refs.list_services_selected.scrollTop = 0
