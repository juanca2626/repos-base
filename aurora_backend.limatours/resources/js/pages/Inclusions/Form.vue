    ...
    <div class="row">
            ...
            <form @submit.prevent="validateBeforeSubmit">
                <div class="b-form-group form-group">
                    <div class="form-row" id="container_country">
                        <label class="col-sm-1 col-form-label" for="inclusion_name">{{ $t('inclusions.inclusion_name')
                            }}</label>
                        <div class="col-sm-4">
                            <input :class="{'form-control':true }"
                                   id="inclusion_name" name="inclusion_name"
                                   type="text"
                                   v-model="form.translations[currentLang].inclusion_name" v-validate="'required'">
                            <div class="bg-danger" style="margin-top: 3px;border-radius: 2px;">
                                <font-awesome-icon :icon="['fas', 'exclamation-circle']"
                                                   style="margin-left: 5px;" v-show="errors.has('inclusion_name')"/>
                                <span v-show="errors.has('inclusion_name')">{{ errors.first('inclusion_name') }}</span>
                            </div>
                        </div>
                ...
                <div class="b-form-group form-group">
                    <div class="form-row">
                        <label class="col-sm-1 col-form-label">Operatividad</label>
                        <div class="col-sm-4">
                            <b-button-group>
                                <b-button @click="changeOpe(form.days.monday,'monday')" :disabled="loading"
                                          :class="{'bg-success': form.days.monday,'bg-danger':!form.days.monday}">L
                                </b-button>
                                <b-button @click="changeOpe(form.days.tuesday,'tuesday')" :disabled="loading"
                                          :class="{'bg-success': form.days.tuesday,'bg-danger':!form.days.tuesday}">M
                                </b-button>
                                <b-button @click="changeOpe(form.days.wednesday,'wednesday')" :disabled="loading"
                                          :class="{'bg-success': form.days.wednesday,'bg-danger':!form.days.wednesday}">M
                                </b-button>
                                <b-button @click="changeOpe(form.days.thursday,'thursday')" :disabled="loading"
                                          :class="{'bg-success': form.days.thursday,'bg-danger':!form.days.thursday}">J
                                </b-button>
                                <b-button @click="changeOpe(form.days.friday,'friday')" :disabled="loading"
                                          :class="{'bg-success': form.days.friday,'bg-danger':!form.days.friday}">V
                                </b-button>
                                <b-button @click="changeOpe(form.days.saturday,'saturday')" :disabled="loading"
                                          :class="{'bg-success': form.days.saturday,'bg-danger':!form.days.saturday}">S
                                </b-button>
                                <b-button @click="changeOpe(form.days.sunday,'sunday')" :disabled="loading"
                                          :class="{'bg-success': form.days.sunday,'bg-danger':!form.days.sunday}">D
                                </b-button>
                            </b-button-group>
                        </div>
                    </div>
        ...
        <div class="col-sm-6">
            <div slot="footer">
                ...
                <button @click="validateBeforeSubmit" class="btn btn-success" type="submit" v-if="!loading && hasClient">
                    <font-awesome-icon :icon="['fas', 'dot-circle']"/>
                    {{ $t('global.buttons.submit') }}
                </button>
            ...
            API.get('/languages/')
                .then((result) => {
                    ...
                    if (this.$route.params.id !== undefined) {
                        ...
                        API.get('/inclusions/' + this.$route.params.id)
                            .then((result) => {
                                ...
                                arrayTranslations.forEach((translation) => {
                                    form.translations[translation.language_id] = {
                                        id: translation.id,
                                        inclusion_name: translation.value
                                    }
                                })

                                form.days.monday = !!(this.inclusion[0].monday)
                                form.days.tuesday = !!(this.inclusion[0].tuesday)
                                form.days.wednesday = !!(this.inclusion[0].wednesday)
                                form.days.thursday = !!(this.inclusion[0].thursday)
                                form.days.friday = !!(this.inclusion[0].friday)
                                form.days.saturday = !!(this.inclusion[0].saturday)
                                form.days.sunday = !!(this.inclusion[0].sunday)
        ...
        methods: {
            ...
            submit () {
                ...
                API({
                    ...
                    url: 'inclusions' + (this.$route.params.id !== undefined ? '/'+this.$route.params.id : ''),
                    data: this.form
                })
                    .then((result) => {
                        if (result.data.success === false) {
                            ...
                            this.$router.push('/inclusions/list')
                        }
                    }).catch(() => {
                    this.$notify({
                        group: 'main',
            ...
            changeOpe(status,day){
              this.$set(this.form.days,day,(status == true) ? false : true)
            }
        }
    }
</script>
